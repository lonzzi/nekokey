...
export const Pager = forwardRef<PagerRef, React.PropsWithChildren<Props>>(function PagerImpl(
  ...
  const handlePageScroll = usePagerHandlers(
    {
      ...
      onPageScrollStateChanged(e: PageScrollStateChangedNativeEventData) {
        ...
        if (parentOnPageScrollStateChanged) {
          runOnJS(parentOnPageScrollStateChanged)(e.pageScrollState);
        }
      },
  ...
  return (
    <View testID={testID} className="flex-1 overflow-hidden">
      {renderTabBar({
        selectedPage,
...
function usePagerHandlers(
  ...
  // @ts-expect-error the type is working
  ...
  return useEvent(
    (event) => {
      ...
      if (event.eventName.endsWith('onPageScroll')) {
        ...
        onPageScrollStateChanged(event as unknown as PageScrollStateChangedNativeEventData);
